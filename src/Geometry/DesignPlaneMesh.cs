...
{
    class DesignPlaneMesh : Geometry
    {
        ...
        public DesignPlaneMesh(ref Mesh mesh)
        {
            triMesh = Triangulate(mesh);
            initMeshGeometry(ref triMesh);
        }

        public DesignPlaneMesh(ref Scene s, ref Mesh mesh)
        {
            mScene = s;
            triMesh = Triangulate(mesh);
            initMeshGeometry(ref triMesh);
        ...
        public DesignPlaneMesh(ref Scene s, OpenTK.Matrix4 t)
        {
            mScene = s;
            mNumPrimitives = 0;
            primitiveType = OpenTK.Graphics.OpenGL4.BeginMode.Triangles;
